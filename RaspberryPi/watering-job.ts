...
class WateringJob {
  ...
  */
  ...
  // User-facing log lines for start/stop and per-relay failures are
  // emitted by ShadowRelay (onActualStateChange + watchdog), not here.
  // The technical logger.error calls in this file stay so the Pi-side
  // tech logs still surface partial-failure diagnostics.
  ...
  private async startWatering(): Promise<void> {
    ...
    logger.error(`startWatering "${this._name}": ${failures.length}/${this._relays.length} relay opens failed`);
  ...
  private async stopWatering(): Promise<void> {
    ...
    for (const result of failures) {
      if (result.status === 'rejected') {
        logger.error(`stopWatering "${this._name}": relay close failed: ${result.reason}`);
      }
    }
  }
}
